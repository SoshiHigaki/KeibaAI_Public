...
class Simulation_Dataset(torch.utils.data.Dataset):
    ...
    def prepare(self, path):
        ...
        race_id = list(df['race_id'])
        horse_number = list(df['horse_number'])
        ...
        self.df['horse_number'] = horse_number
...
#         self.input_columns = [f'past_{i+1}' for i in range(self.horse_dataset.horse_past_number)]
#         self.output_columns = ['race']
...
#         horse_df = pd.DataFrame(horse_tensor).apply(self.horse_fillna, axis=1)
#         horse_df.columns = self.input_columns

#         self.df = pd.concat([race_df, horse_df], axis=1)
